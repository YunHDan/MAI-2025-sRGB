...
def multi_device_paired_paths(folders_list, keys, filename_tmpl='{}'):
    """为多设备生成独立配对的路径，允许不同设备有同名文件

    Args:
        folders_list: [(input_folder1, gt_folder1), ...]，每组对应一个设备
        keys: 如 ['lq', 'gt']
        filename_tmpl: 输入文件到GT文件的名称映射模板（默认直接同名）

    Returns:
        list[dict]: 包含所有设备配对数据的列表
    """
    assert len(keys) == 2, "keys必须为[lq_key, gt_key]"
    input_key, gt_key = keys

    all_pairs = []

    # 遍历每个设备的输入-GT文件夹组
    for input_folder, gt_folder in folders_list:
        # 获取排序后的文件名列表（确保对齐）
        input_files = sorted([f.name for f in scandir(input_folder) if f.is_file()],
                             key=lambda x: int(osp.splitext(x)[0]))  # 按数字排序
        gt_files = sorted([f.name for f in scandir(gt_folder) if f.is_file()],
                          key=lambda x: int(osp.splitext(x)[0]))

        # 验证文件数量匹配
        assert len(input_files) == len(gt_files), (
            f"文件数不匹配: {input_folder}({len(input_files)}) vs {gt_folder}({len(gt_files)})"
        )

        # 逐对处理
        for input_file, gt_file in zip(input_files, gt_files):
            # 提取基础名（不带扩展名）
            input_basename = osp.splitext(input_file)[0]
            gt_basename = osp.splitext(gt_file)[0]

            # 验证GT文件名是否符合模板预期
            expected_gt_basename = filename_tmpl.format(input_basename)
            assert gt_basename == expected_gt_basename, (
                f"GT文件不匹配: {gt_basename} ≠ 预期 {expected_gt_basename}\n"
                f"输入文件: {input_file}\n模板: '{filename_tmpl}'"
            )

            # 构建完整路径
            input_path = osp.join(input_folder, input_file)
...
class Dataset_PairedImage(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.device_folders = [
            ('dped/blackberry/training_data/blackberry', 'dped/blackberry/training_data/canon'),
            ('dped/iphone/training_data/iphone', 'dped/iphone/training_data/canon'),
            ('dped/sony/training_data/sony', 'dped/sony/training_data/canon')
        ]
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            if self.opt['phase'] == 'train':
                self.paths = multi_device_paired_paths(
                    self.device_folders, ['lq', 'gt'],
                    self.filename_tmpl)
            else:
